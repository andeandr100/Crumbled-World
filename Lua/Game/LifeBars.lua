...
function LifeBar.new()
	...
	local function init()
		...
		camera = rootNode:findNodeByName("MainCamera")
		...
		rootNode:addChild(nodeMesh:toSceneNode())
		...
		if nodeMesh:getVisible() and targetSelector.disableRealityCheck() then
			...
			while i<=soulList.count do
				...
				if soul.hp==soul.hpMax and isIgnored == false and isHighPrio == false then
					...
					if showOnlyDamagedNpcs and targetSelector.getTargetStateValue(soul.index) < 1.0 then
			...
			if lifeBarCount ~= souls or souls > 0 then
				...
				for i=1, souls do
					local soul = soulList[i]
					positionList[i] = Vec4( soul.oldPosition + positionOffset,soul.hp/soul.hpMax)
					normalList[i] = Vec4(0.0,0.0,showDamageValue,targetSelector.getTargetStateValue(soul.index))
					indexList[i] = i - 1
				end
