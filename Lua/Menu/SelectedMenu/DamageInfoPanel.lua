...
function DamageInfoPanel.new(inParentPanel)
	...
	local function getDamageToolTipText()
		...
		local damageToolTip = Text(tostring(math.round(damage/totalCost)).." ") + language:getText("DamageToolTip.damage per gold") + Text("\n")
		damageToolTip = damageToolTip + Text(tostring(math.round(damage)).." ") + language:getText("DamageToolTip.damage delt to enemies") 
		...
		if buildingBillBoard:exist("DamagePreviousWavePassive") then
			...
			if passivDamage > 1 then
				...
				damageToolTip = damageToolTip + Text(tostring(math.round(passivDamage/totalCost)).." ") + language:getText("DamageToolTip.damage per gold") + Text("\n")
				damageToolTip = damageToolTip + Text(tostring(math.round(passivDamage)).." ") + language:getText("DamageToolTip.damage delt to enemies") 
			end
		end
		...
		if buildingBillBoard:exist("goldEarned") then
			...
			if goldEarned > 1 then
				...
				damageToolTip = damageToolTip + Text(tostring(goldEarned).." ") + language:getText("DamageToolTip.gold earned") + Text("\n")
				damageToolTip = damageToolTip + Text(tostring(goldEarnedPreviousWave).." ") + language:getText("DamageToolTip.gold earned previous wave") 
			end
		end
