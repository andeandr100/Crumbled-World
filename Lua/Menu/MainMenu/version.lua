...
function create()
	...
	form = Form( versionCamera, PanelSize(Vec2(-1,-1)), Alignment.TOP_LEFT, "VersionForm");
	...
	if model then
		...
		if camera then
			form = Form(ConvertToCamera(camera), PanelSize(Vec2(1,0.025)), Alignment.BOTTOM_LEFT, "VersionCameraForm");
